 ...
 *
 * Usage:
 * - `/review` - show interactive selector (or review all workspace files when not in a git repo)
...
type ReviewTarget =
	...
	| { type: "workspace"; ignoredGlobs: string[] };
...
const WORKSPACE_REVIEW_PROMPT =
	"Review the entire workspace as a snapshot (not a diff). Recursively review source files under the current working directory while ignoring these paths/patterns: {ignoredGlobs}. Skip binary/media/generated artifacts and focus on actionable code-quality, correctness, security, and maintainability findings.";

const NON_GIT_REVIEW_IGNORE_GLOBS = [
	".git/**",
	"node_modules/**",
	".venv/**",
	"venv/**",
 ...
 */
async function isGitRepository(pi: ExtensionAPI): Promise<boolean> {
...
async function buildReviewPrompt(pi: ExtensionAPI, target: ReviewTarget): Promise<string> {
	switch (target.type) {
		...
		case "workspace":
			return WORKSPACE_REVIEW_PROMPT.replace("{ignoredGlobs}", target.ignoredGlobs.join(", "));
	}
}
...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			const inGitRepo = await isGitRepository(pi);
			...
			if (!inGitRepo) {
				if (parsed) {
					if (parsed.type === "folder" || parsed.type === "custom") {
						target = parsed;
					} else {
						ctx.ui.notify(
							"This review mode requires git history. Falling back to reviewing all workspace files.",
							"warning",
						);
					}
				}
				...
				if (!target) {
					target = { type: "workspace", ignoredGlobs: [...NON_GIT_REVIEW_IGNORE_GLOBS] };
					ctx.ui.notify(
						"Not a git repository. Reviewing all workspace files with default ignore patterns.",
						"info",
					);
				}
			} else {
